...
class Navbar {
    ...
    render() {
        return `
                    ...
                    <div class="container-sm d-flex flex-sm-column">
                        <div class="row d-flex flex-column g-2 w-100">
                            <div class="col-sm-12 d-flex justify-content-center">
                                <div id="search-box-offcanvas" class="d-flex search-box">
                                    <svg class="search-lens" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                                        <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
                                    </svg>
                                    <input id="search-bar-offcanvas" class="form-control me-2 search-bar" placeholder="Cerca Immagini" aria-label="Search">
                                </div>
                            </div>
                            ${
                                this.user ? `
            ...
            <a id="logo-title" class="navbar-brand" href="/home">
                <svg id="logo" class="${this.user ? 'no-margin-right margin-left' : ''}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
            ...
            <div id="search-box-navbar" class="d-flex search-box">
                <svg class="search-lens" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16">
                    <path d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001c.03.04.062.078.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1.007 1.007 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0z"/>
                </svg>
                <input id="search-bar-navbar" class="form-control me-2 search-bar" placeholder="Cerca per Titolo, Artista, Genere o Tag" aria-label="Search">
            </div>
            ${
                this.user ? `
    ...
    attachEventListeners() {
        ...
        const searchBar = document.getElementById('search-bar-navbar');
        const searchBarOffcanvas = document.getElementById('search-bar-offcanvas');
        ...
        if (searchBar) {
            searchBar.addEventListener('keyup', (e) => {
                e.preventDefault();
                if(e.key === 'Enter') {
                    window.location.href = `/search?value=${searchBar.value}&searchBy=all&orderBy=date`;
                } else e.stopPropagation();
            });
        }
        if (searchBarOffcanvas) {
            searchBarOffcanvas.addEventListener('keyup', (e) => {
                e.preventDefault();
                if(e.key === 'Enter') {
                    window.location.href = `/search?value=${searchBarOffcanvas.value}&searchBy=all&orderBy=date`;
                } else e.stopPropagation();
            });
        }
